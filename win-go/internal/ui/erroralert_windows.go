...
// Verbatim English messages from docs/specs/funcspec.md Р’В§ Errors (user-facing).
const (
...
func (a *app) showErrorAlert(code, message string) {
	...
	walk.MsgBox(a.mw, "Erase & Rewrite", text, walk.MsgBoxOK|walk.MsgBoxApplModal|walk.MsgBoxIconError)
}
...
func (a *app) showInterruptionAlert(message string) {
	...
	walk.MsgBox(a.mw, "Erase & Rewrite", message, walk.MsgBoxOK|walk.MsgBoxApplModal|walk.MsgBoxIconInformation)
}

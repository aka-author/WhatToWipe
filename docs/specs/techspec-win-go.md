# Erase & Rewrite: Technical Specification (Windows, Go)
...
Words like РІР‚СљmustРІР‚Сњ, РІР‚Сљmust notРІР‚Сњ, and РІР‚СљshouldРІР‚Сњ carry their usual English force: РІР‚СљmustРІР‚Сњ means the implementation is not compliant if it fails the point; РІР‚СљshouldРІР‚Сњ is a strong recommendation. This file does not use the RFC 2119 keyword capitalization style.
...
| ID | Requirement |
|----|-------------|
| WR-01 | The release executable must be a normal Windows PE that can carry the metadata Windows Explorer shows under Properties РІвЂ вЂ™ Details, so the FS rules that tie About to file version can be met in the usual way. |
| WR-02 | The build must embed a `VERSIONINFO` resource with string file info populated so ExplorerРІР‚в„ўs Details view is meaningful. At minimum the `FileVersion` and `ProductVersion` string fields must be set in a way that matches how you read them back for the FS About string; other common fields (`CompanyName`, `FileDescription`, `LegalCopyright`, `OriginalFilename`, `ProductName`) should be filled for a shipping binary so support staff can identify the build. The exact About text remains defined only in FS. |
...
How you obtain РІР‚Сљtotal capacity of the driveРІР‚Сњ for FS drive share is not spelled in FS; you must use one consistent Windows API interpretation (for example total bytes on the volume that holds the scan root) and document it for verification.
...
| ID | Requirement |
|----|-------------|
| RS-01 | Stop and Esc must remain usable while scanning; the UI thread must not be wedged for so long that the user cannot invoke them. There is no fixed millisecond budget here, but РІР‚Сљindefinite hangРІР‚Сњ is a defect. |
...
| ID | Requirement |
|----|-------------|
| VR-01 | Each requirement row in Р’В§5 through Р’В§12 must have a written test plan entry, an automated test, or an explicit waiver before release. |
| VR-02 | Any waiver against a РІР‚СљmustРІР‚Сњ in this document must be explicit, with a short reason and a plan to get back to compliance. |
...
For reviewers: keep one short note in the repo (path up to you) that records the shipped choices for IO-01 long-path mechanism, IO-03 reparse policy, drive-total API for share math, and logging per architecture Р’В§7, so VR-01 evidence is easy to find.

...
The main window icon is broombunny art. Native-size masters live in `assets/icons/broombunny-{16,20,24,32,40,48,64,256}.png`, generated by `tools/generate_icon_masters.cpp` (foreground crop on the high-res source before downscale). `tools/build_app_ico.cpp` packs those masters into `app.ico` (BMP ≤64, PNG 256) without resizing. On Windows the main window uses native `WM_SETICON` via `applyWindowIcons()` at `WinIdChange`, `Show`, and `WM_DPICHANGED`, loading PE resource 1 at `SM_CXSMICON`/`SM_CXICON` metrics. Qt `setWindowIcon()` is disabled for the main window on Windows; About uses `QIcon` from `:/app/icons/`. `SetCurrentProcessExplicitAppUserModelID` (`MikhailOstrogorskiy.EraseAndRewrite`) runs before `QApplication`.
...
| Asset | Source |
|-------|--------|
| `app.ico` | `assets/art/broombunny*.png` via `win-cpp-qt/tools/build_app_ico.cpp` during `build.ps1` |
